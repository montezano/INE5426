...
LITERAL : ( DIGITO+ | VERDADEIRO | FALSO | TEXTO ) ;
...
expressao : (DIGITO | FUNCAO | IDENTIFICADOR (MAISMAIS | MENOSMENOS)? | TEXTO) (OPERADORES_MATEMATICOS (DIGITO | FUNCAO | ( IDENTIFICADOR (MAISMAIS | MENOSMENOS)? | TEXTO)))* ; 

atribuicao : QUALIFICADOR? TIPO IDENTIFICADOR (ATRIBUICAO expressao)?  PONTOVIRGULA;
...
caso: CASO LITERAL DOISPONTOS (logica_da_aplicacao);
...
logica_da_aplicacao : (chamada_funcao_classe | chamada_funcao_servico | atribuicao | operacao_matematica | se | para | enquanto | escolha |
                      RETORNO (LITERAL | IDENTIFICADOR ) ) PONTOVIRGULA;